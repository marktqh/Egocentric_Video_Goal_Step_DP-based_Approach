...
class VideoTextAlignmentDataset(Dataset):
    def __init__(self, video_root_dir, annotation_file, device='cpu', negative_sampling_ratio=1):
        ...
        self.negative_sampling_ratio = max(1, int(negative_sampling_ratio))  # Ensure ratio is at least 1
        self.chunk_index = self.create_chunk_index()
        self.prev_neg_is_text = False
    ...
    def get_available_videos(self):
        ...
        for video_uid in self.annotation_data.keys():
            ...
            if os.path.exists(video_path):
                ...
                available_videos.append(video_uid)
        ...
        for video_uid in self.available_videos:
            ...
            for segment in segments:
                ...
                for chunk_idx in range(start_chunk, end_chunk + 1):
                    chunk_index.append((video_uid, chunk_idx, segment['step_description']))
        return chunk_index
    ...
    def __getitem__(self, idx):
        try:
            # Get video_uid, chunk index, and step description
            ...
            if self.negative_sampling_ratio == 1:
                if self.prev_neg_is_text:
                    video_side_negative = 1
                    text_side_negative = 0
                else:
                    video_side_negative = 0
                    text_side_negative = 1
                self.prev_neg_is_text = not self.prev_neg_is_text   
            else:
                video_side_negative = int(self.negative_sampling_ratio / 2)
                text_side_negative = self.negative_sampling_ratio - video_side_negative
            ...
            attempt_count = 0
            while video_side_negative_count + text_side_negative_count < self.negative_sampling_ratio:
                attempt_count += 1
                random_video_uid = random.choice(self.available_videos)

                # if attempt_count > max_attempts:
                #     print('Max retrieve count reached.')
                #     break
                ...
                if text_side_negative_count < text_side_negative:
                    if random_video_uid == video_uid:
                        if len(self.annotation_data[random_video_uid]['segments']) > 1:
                            ...
                            if text_side_negative_count + video_side_negative_count >= self.negative_sampling_ratio:
                                break
                else:
                    ...
                    if random_video_uid == video_uid:
                        if len(self.annotation_data[random_video_uid]['segments']) > 1:
                            ...
                            try:
                                ...
                                print(f'Video {random_video_uid} chunk {random_chunk_idx} not found.')
                                continue
                    else:
...
def main(batch_size=8):
    ...
    # annotation_file = r'.\ego4d_data\V5\annotations\goalstep_val.json'  # Path to annotation file
    ...
    with open(f'/scratch/qt2087/DSGA1006_code/logs/V5_{batch_size}_{dataset.negative_sampling_ratio}.log', 'a') as f:
        f.write(f"Dataset size: {len(dataset)}; Batch size: {batch_size}; Number of bathces: {total_batches}.\n")
    f.close()
    print(f"lr will be decreased every {lr_step} batches with scale {lr_scale}.")
    
    print("Starting training...")

    writer = SummaryWriter(f'/scratch/qt2087/DSGA1006_code/logs/V5')
    scaler = GradScaler()
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            if (batch_idx+1) % lr_step == 0:
                for param_group in optimizer.param_groups:
                    param_group['lr'] *= lr_scale
            if (batch_idx+1) % 10 == 0:
                model_dir = f'/scratch/qt2087/DSGA1006_code/lavila_adapter_models/V5/{batch_size}_{dataset.negative_sampling_ratio}'
                os.makedirs(model_dir, exist_ok=True)
                torch.save(model.state_dict(), os.path.join(model_dir, f'alignment_model_{epoch}_{batch_idx}.pth'))
            prev_batch_time = time()
        ...
        model_dir = f'/scratch/qt2087/DSGA1006_code/lavila_adapter_models/V5/{batch_size}_{dataset.negative_sampling_ratio}'
        os.makedirs(model_dir, exist_ok=True)
        torch.save(model.state_dict(), os.path.join(model_dir, f'video_text_alignment_model_V5_{epoch}_all.pth'))
    ...
    model_dir = f'/scratch/qt2087/DSGA1006_code/lavila_adapter_models/V5/{batch_size}_{dataset.negative_sampling_ratio}'
    os.makedirs(model_dir, exist_ok=True)
    torch.save(model.state_dict(), os.path.join(model_dir, f'video_text_alignment_model_V5.pth'))
