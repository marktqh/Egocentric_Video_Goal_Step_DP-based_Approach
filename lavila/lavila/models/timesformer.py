...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        # print("Adapter layer activated")
        return x + self.up_proj(torch.relu(self.down_proj(x)))  # Residual connection
...
class SpaceTimeTransformer(nn.Module):
    ...
    def forward_features(self, x, use_checkpoint=False, cls_at_last=True):
        ...
        # Iterate over blocks and apply adapters
        for i, blk in enumerate(self.blocks):
            x = blk(x, self.einops_from_space, self.einops_to_space, self.einops_from_time,
                    self.einops_to_time, time_n=n, space_f=f, use_checkpoint=use_checkpoint)
            
            # Apply adapter if it exists for this block
            if hasattr(self, 'adapters') and isinstance(self.adapters[i], Adapter):
                x = self.adapters[i](x)
